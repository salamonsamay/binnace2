...
public class ExtractStepSize {
    ...
    public  static  int extract(String pair) {
        ...
        try {
            ...
            // Assuming you want to extract stepSize from the first element in the symbols array
            ...
            if (symbolsArray.isArray() && symbolsArray.size() > 0) {
                ...
                if (filtersArray.isArray() && filtersArray.size() > 0) {
                    ...
                    return  countDiggitAfterDot(firstFilter.get("stepSize").asText());
    ...
    public static double truncateDecimal(double quntity, int y) {
        if (y < 0) {
    ...
    private   static int countDiggitAfterDot(String num){
